...
## Problem 1: Seeded Version

The following code is the same as the code in the Lab assignment, but here we **seed** the random number generator so it produces the same exact results every time it is run. (This makes it easier to refer to "the" plots in the problem, when helping in office hours: using `set.seed(5100)` ensures that the plots are the same between students' and teachers' computers!)
